...
window.fetch = (input, init = {}) => {
  ...
  if (!['GET', 'HEAD', 'OPTIONS'].includes(method)) {
    if (adminUser.id) headers.set('X-Admin-Id', encodeHeaderValue(adminUser.id));
    if (adminUser.username) headers.set('X-Admin-Username', encodeHeaderValue(adminUser.username));
    if (adminUser.nickname) headers.set('X-Admin-Name', encodeHeaderValue(adminUser.nickname));
  }
