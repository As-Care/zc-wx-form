  ...
  >
    <a-spin :loading="loading" style="width: 100%;">
      <section class="form-section">
        ...
        <a-form :model="form" layout="vertical">
          ...
          <a-form-item v-if="addresses.length" label="客户已保存地址">
            <a-select v-model="selectedAddressId" placeholder="请选择地址" @change="handleAddressChange">
              <a-option v-for="address in addresses" :key="address.id" :value="address.id">
                {{ address.is_default ? '【默认】' : '' }}{{ formatAddress(address) }}
              </a-option>
            </a-select>
          </a-form-item>
          <a-form-item label="安装详细地址" required>
            <a-textarea
              ...
              :placeholder="addresses.length ? '可选择已有地址，也可以手动修改' : '该客户没有默认地址，请管理员填写安装详细地址'"
      ...
      <section class="form-section">
        ...
        <a-form layout="vertical">
          <a-form-item label="门窗商品" required>
            <a-select v-model="form.product_id" placeholder="请选择商品" allow-search @change="handleProductChange">
        ...
        <div v-for="(set, setIndex) in customSets" :key="set.id" class="set-panel">
          <div class="set-header">
            <strong>配置 {{ setIndex + 1 }}</strong>
            <a-button v-if="customSets.length > 1" type="text" status="danger" size="small" @click="removeSet(setIndex)">
          ...
          <a-grid :cols="3" :col-gap="12">
            ...
            <a-grid-item>
              <a-form-item label="宽度（mm）" required>
                <a-input-number v-model="set.width_mm" :min="1" :precision="0" style="width: 100%;" />
              </a-form-item>
            </a-grid-item>
            <a-grid-item>
              <a-form-item label="高度（mm）" required>
                <a-input-number v-model="set.height_mm" :min="1" :precision="0" style="width: 100%;" />
              </a-form-item>
            </a-grid-item>
          </a-grid>

          <a-form-item v-for="group in productDetail.optionGroups || []" :key="group.group_name" :label="group.groupTitle">
            <a-radio-group v-model="set.selected_options[group.group_name]">
              <a-radio v-for="option in group.options" :key="option.id" :value="option.id">
                {{ option.option_name }}<span class="option-price">{{ option.priceText }}</span>
              </a-radio>
            </a-radio-group>
          </a-form-item>

          <a-form-item label="本套现场备注">
            <a-textarea v-model="set.customer_remark" placeholder="测量、安装环境或客户要求" :auto-size="{ minRows: 2, maxRows: 3 }" />
          </a-form-item>
      ...
      <section class="form-section">
        ...
        <a-grid :cols="2" :col-gap="16">
          ...
          <a-grid-item>
            <div class="total-price">预估总金额 <strong>¥{{ totalPrice.toFixed(2) }}</strong></div>
          </a-grid-item>
        </a-grid>
        <a-form-item label="后台备注">
          <a-textarea v-model="form.admin_remark" placeholder="仅后台查看的订单备注" :auto-size="{ minRows: 2, maxRows: 4 }" />
        </a-form-item>
      </section>
    </a-spin>
    ...
    <template #footer>
      <a-space>
        ...
        <a-button type="primary" :loading="submitting" @click="submitOrder">创建订单</a-button>
      </a-space>
    </template>
  </a-drawer>
</template>
...
const form = ref({
  user_id: '', customer_name: '', customer_phone: '', install_address: '',
  product_id: '', status: 'pending_review', admin_remark: ''
});
...
const filterCustomer = (input, option) => String(option.label || option.value || '').toLowerCase().includes(input.toLowerCase());
const formatAddress = address => `${address.province || ''}${address.city || ''}${address.district || ''}${address.detail_address || ''}`;
...
const handleCustomerChange = async (userId) => {
  ...
  selectedAddressId.value = '';
  try {
    const res = await fetch(`${API_BASE}/api/user/addresses?user_id=${encodeURIComponent(userId)}`);
    ...
    if (defaultAddress) {
      ...
      form.value.customer_name = defaultAddress.name || form.value.customer_name;
      form.value.customer_phone = defaultAddress.phone || form.value.customer_phone;
    }
  } catch (e) {
    addresses.value = [];
  }
...
const createSet = index => {
  ...
  (productDetail.value.optionGroups || []).forEach(group => {
    const defaultOption = group.options.find(option => Number(option.is_default) === 1) || group.options[0];
...
const addSet = () => customSets.value.push(createSet(customSets.value.length + 1));
const removeSet = index => customSets.value.splice(index, 1);

const getSelectedOptions = set => {
  if (!productDetail.value) return [];
  return (productDetail.value.optionGroups || []).map(group => {
    const selectedId = set.selected_options[group.group_name];
    const option = group.options.find(item => item.id === selectedId) || group.options[0];
    return option ? { ...option, group_name: group.group_name, groupTitle: group.groupTitle } : null;
  }).filter(Boolean);
};

const calculateSet = set => {
  if (!productDetail.value) return { billedArea: 0, totalPrice: 0 };
  const actualArea = Number(set.width_mm || 0) * Number(set.height_mm || 0) / 1000000;
  const billedArea = Math.max(actualArea, Number(productDetail.value.min_area || 1));
...
const totalPrice = computed(() => customSets.value.reduce((sum, set) => sum + calculateSet(set).totalPrice, 0));

const handleSceneUpload = (setIndex, fileItem) => {
  const url = fileItem?.response?.url || fileItem?.url;
  if (url && customSets.value[setIndex] && !customSets.value[setIndex].scene_images.includes(url)) {
...
const validate = () => {
  ...
  if (!/^1[3-9]\d{9}$/.test(form.value.customer_phone.trim())) return '请填写有效联系电话';
  if (!form.value.install_address.trim()) return '请填写安装详细地址';
  if (!productDetail.value || !customSets.value.length) return '请选择商品并填写配置';
  for (let index = 0; index < customSets.value.length; index += 1) {
    ...
    if (Number(set.width_mm) <= 0 || Number(set.height_mm) <= 0) return `请填写配置 ${index + 1} 的有效宽高`;
  }
  return '';
};
  ...
  try {
    ...
    const res = await fetch(`${API_BASE}/api/orders`, {
      ...
      body: JSON.stringify({
        ...
        customSets: sets,
        customer_remark: sets.map(set => set.customer_remark).filter(Boolean).join('；'),
    ...
    const data = await res.json();
    if (!res.ok || !data.success) throw new Error(data.message || `创建失败 (HTTP ${res.status})`);
...
.product-summary { display: flex; gap: 20px; align-items: center; padding: 12px 0; color: var(--color-text-2); }
.set-panel { padding: 16px 0; border-top: 1px solid var(--color-border-2); }
.set-header { display: flex; align-items: center; justify-content: space-between; margin-bottom: 12px; }
.option-price { margin-left: 6px; color: #b89768; font-size: 12px; }
.set-price { text-align: right; color: var(--color-text-2); }
.set-price strong { margin-left: 8px; color: #b89768; font-size: 17px; }
.total-price { height: 64px; display: flex; align-items: center; justify-content: flex-end; gap: 10px; color: var(--color-text-2); }
.total-price strong { color: #b89768; font-size: 22px; }
</style>
