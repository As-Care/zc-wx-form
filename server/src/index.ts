...
async function initProductsTable(db: any) {
  ...
  try {
    ...
    `,
      ...
      .run();
    await db
      .prepare("ALTER TABLE products ADD COLUMN category_name TEXT")
      .run();
...
async function initProductOptionsTable(db: any) {
  ...
  try {
    ...
    try {
      await db
        .prepare("ALTER TABLE product_options ADD COLUMN group_title TEXT")
        .run();
    } catch (e) {}
    try {
      await db
        .prepare("ALTER TABLE product_options ADD COLUMN image_url TEXT")
        .run();
    } catch (e) {}
...
async function initOrdersTable(db: any) {
  ...
  try {
    await db
      .prepare("ALTER TABLE orders ADD COLUMN install_address TEXT")
      .run();
  } catch (e) {}
  try {
    await db
      .prepare("ALTER TABLE orders ADD COLUMN customer_remark TEXT")
      .run();
  } catch (e) {}
  try {
  ...
  try {
    await db
      .prepare("ALTER TABLE order_items ADD COLUMN scene_images TEXT")
      .run();
  } catch (e) {}
...
app.delete("/api/orders/:id", async (c) => {
  ...
  try {
    ...
    // 删除订单日志
    await db.prepare("DELETE FROM order_logs WHERE order_id = ?").bind(id).run();
    // 删除主订单
...
async function initRBACTables(db: any) {
  ...
  try {
    ...
    const countMenus = await db
      .prepare("SELECT COUNT(*) as count FROM sys_menus")
      .first<{ count: number }>();
    if (!countMenus || countMenus.count === 0) {
      ...
      for (const m of defaultMenus) {
        ...
        `,
          ...
          .bind(m.id, m.key, m.name, m.path, m.icon, m.sort_order)
          .run();
      }
    }
    ...
    // 给 root 角色绑定全量菜单权限
    const { results: allMenus } = await db
      .prepare("SELECT key FROM sys_menus")
      .all<{ key: string }>();
